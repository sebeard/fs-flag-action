...
class FlagsmithAPI {
  ...
  constructor({
    baseUrl, environmentKey, projectId, apiToken,
  }) {
...
const FlagsmithAPI = __nccwpck_require__(2412);
const { filteredFileList } = __nccwpck_require__(4051);
const { getParentDirectoryName } = __nccwpck_require__(7047);
const { getFileContents } = __nccwpck_require__(1801);
...
exports.getFileContents = async function getFileContents(filePath) {
  core.debug(`Reading contents of ${filePath}`);
  return readFileSync(path.join(__dirname, filePath), { encoding: "utf8" });
};
...
const { getInputOrDefault, getRequiredInput } = __nccwpck_require__(8719);
...
exports.getInputs = () => {
  try {
    ...
    const fileName = getInputOrDefault('fileName', /^\w+$/, 'flagValue.json');
...
const path = __nccwpck_require__(1017);
const { readdir } = (__nccwpck_require__(7147).promises);
const core = __nccwpck_require__(2186);
...
exports.filteredFileList = async function filteredFileList({
  ...
  await readdir(path.join(__dirname, dirName), { withFileTypes: true }).then(
    (items) => {
      items.forEach((item) => {
        if (item.isDirectory()) {
          core.debug(`Searching in subdirectory ${dirName}/${item.name}`);
          subDirectoryPromises.push(
            filteredFileList(`${dirName}/${item.name}`, fileName),
          );
